...
class eZCreateContent extends eZXMLInstallerHandler
{
    ...
    function proccessCreateContent( $xmlNode, $parentNodeID = false )
    {
        if ( !$parentNodeID )
        {
            $parentNodeID = $xmlNode->attributeValue( 'parentNode' );
            if ( !$parentNodeID )
            {
        ...
        foreach ( $objectList as $objectNode )
        {
            ...
            $objectInformation['classID'] = $objectNode->attributeValue( 'contentClass' );
            $objectInformation['remoteID'] = $objectNode->attributeValue( 'remoteID' );
            $objectInformation['objectID'] = $objectNode->attributeValue( 'objectID' );
            $objectInformation['sectionID'] = $objectNode->attributeValue( 'section' );
            $objectInformation['ownerID'] = $objectNode->attributeValue( 'owner' );
            $objectInformation['creatorID'] = $objectNode->attributeValue( 'creator' );
            $objectInformation['attributes'] = array();
            $attributeObject = $objectNode->elementByName( 'Attributes' );
            if ( $attributeObject )
            {
                $attributes = $attributeObject->children();
                foreach ( $attributes as $attribute )
                {
                    $objectInformation['attributes'][$attribute->name()] = array();
                    $objectInformation['attributes'][$attribute->name()]['content'] = $attribute->textContent();
                    if ( $attribute->hasAttributes() )
                    {
                        $objectInformation['attributes'][$attribute->name()] = array_merge( $attribute->attributeValues(), $objectInformation['attributes'][$attribute->name()] );
                    }
                }
            }
            ...
            {
                $attributes = $reference->attributeValues( );
                if ( array_key_exists( $attributes['attribute'], $refInfo ) )
                {
            ...
            $childs = $objectNode->elementByName( 'Childs' );
            if ( $childs )
            {
